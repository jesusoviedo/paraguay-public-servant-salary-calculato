...
def create_ec2_instance(key_name, security_group_name, security_group_description, image_id, vpc_id, instance_name):
...
def stop_instance_by_name(instance_name):
    ...
    try:
        ...
        print(f"Instance '{instance_name}' ({instance_id}) stopped successfully.")
        return response
    except botocore.exceptions.ClientError as error:
...
def start_instance_by_name(instance_name):
    ...
    try:
        ...
        print(f"Instance '{instance_name}' ({instance_id}) started successfully.")
        return response
    except botocore.exceptions.ClientError as error:
...
def terminate_instance_by_name(instance_name):
    ...
    try:
        ...
        print(f"Instance '{instance_name}' ({instance_id}) terminated successfully.")
        return response
    except botocore.exceptions.ClientError as error:
...
def delete_security_group_by_name(group_name):
    ...
    try:
        ...
        print(f"Security group '{group_name}' ({group_id}) deleted successfully.")
        return response
    except botocore.exceptions.ClientError as error:
        if error.response['Error']['Code'] == 'DependencyViolation':
            print(f"Error: Security group '{group_name}' is still in use. Detach it from any resources before deleting.")
        else:
...
if __name__ == '__main__':
    ...
    security_group_description = os.environ.get('AWS_EC2_SECURITY_GROUP_DESCRIPTION')
    ...
    if task == TASK1:
        ...
        time.sleep(SEGUNDOS)
        public_dns= create_ec2_instance(key_name, security_group_name, security_group_description, image_id, vpc_id, instance_name)

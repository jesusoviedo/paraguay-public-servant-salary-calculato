...
def read_and_split_data():
    ...
    return train_test_split(
        X_data, y_data, test_size=TEST_PERCENTAGE, random_state=RANDOM_STATE
    )
...
def registre_model(run_id, path):
    ...
    model_version = mlflow.register_model(f"runs:/{run_id}/{path}", MODEL_NAME)
    client.set_registered_model_alias(
        name=MODEL_NAME, alias=ALIAS, version=model_version.version
    )
    client.set_model_version_tag(
        name=MODEL_NAME, version=model_version.version, key=TAG, value=TAG_VALUE
    )

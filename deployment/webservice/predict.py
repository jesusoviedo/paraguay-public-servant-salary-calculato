...
def _search_path_prefix_and_version():
    ...
    data_registered_models = [model.aliases.get(ALIAS) for model in client.search_registered_models(f"name='{MODEL_NAME}'")]
    version_n = data_registered_models[0] if data_registered_models else None


    if version_n:
        os.environ["VERSION_PREDICT"] = version_n
        filtered_models = [model for model in client.search_registered_models(f"name='{MODEL_NAME}'") 
                                if any(version.tags.get(TAG_KEY) == TAG_VALUE 
                                    for version in model.latest_versions if version.version == version_n
                                )
        ]
...
def _format_response(pred, ver, attri):
    ...
    result = OrderedDict([('salary', pred), 
                          ('input', attri),
                          ('version', ver), 
